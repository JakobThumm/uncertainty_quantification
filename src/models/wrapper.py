...
def model_from_string(
    ...
    if model_name == "MLP":
        model = MLP(output_dim=output_dim, num_layers=mlp_num_layers, hidden_dim=mlp_hidden_dim, act_fn=act_fn)
        wrapped_model = wrap_model(model)

    elif model_name == "LeNet":
        model = LeNet(output_dim=output_dim, act_fn=act_fn)
        wrapped_model = wrap_model(model)

    elif model_name == "LeNet_h":
        model = LeNet_h(output_dim=output_dim, act_fn=act_fn)
        wrapped_model = wrap_model(model)

    elif model_name == "GoogleNet":
        model = GoogleNet(output_dim=output_dim, act_fn=act_fn)
        ...
        model = ResNet(
            output_dim=output_dim, c_hidden=(16, 32, 64), num_blocks=(3, 3, 3), act_fn=act_fn, block_class=ResNetBlock
        )
        wrapped_model = wrap_model_with_batchstats(model)
        ...
        CONFIG = EasyDict(
            {
                ...
                "TEST": {"FLIP_TEST": True, "HEATMAP2COORD": "coord"},
                "LOSS": {"TYPE": "RLELoss"},
            }
        )
        ...
        model = RegressFlowFlax(preset_cfg=cfg["PRESET"], fc_filters=cfg["NUM_FC_FILTERS"], accept_nchw=True)
        wrapped_model = wrap_model_with_batchstats(model)
        ...
        model = RegressFlowFlaxWithAleatoric(
            preset_cfg=cfg["PRESET"], fc_filters=cfg["NUM_FC_FILTERS"], accept_nchw=True
        )
        wrapped_model = wrap_model_with_batchstats(model)
