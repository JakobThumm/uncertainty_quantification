...
class UncertaintyEmbedding(nn.Module):
    """
    Process input uncertainties parallel to main network (Experiment3 architecture).
    Simple embedding that matches the trained checkpoint architecture.

    Args:
        uncertainty_dim: Dimension of flattened uncertainty input (num_joints * 6)
    ...
    def __call__(self, uncertainty):
        ...
        Args:
            uncertainty: Input uncertainty features [batch_size, uncertainty_dim]
                        where uncertainty_dim = num_joints * 6 (3 vars + 3 covs per joint)

        Returns:
            Scaled uncertainty embeddings [batch_size, d_model]
        """
        # Simple embedding: uncertainty_dim -> d_model (matches checkpoint)
        x = nn.Dense(self.d_model, name="uncertainty_embed_0")(uncertainty)
        x = nn.LayerNorm(name="uncertainty_embed_1")(x)
        x = nn.gelu(x)
...
# DCT/IDCT functions removed - not used in Experiment3 architecture
...
class DCTPoseTransformer(nn.Module):
    ...
    def __post_init__(self) -> None:
        # Experiment3 doesn't use DCT/IDCT transforms
    ...
    def __call__(self, x, train: bool = True):
        """
        Forward pass through the model (Experiment3 architecture - NO DCT/IDCT).

        Args:
            x: Input pose sequence [batch_size, seq_len, input_dim]
               OR with uncertainty: [batch_size, seq_len, input_dim + uncertainty_dim]
        ...
        # Check if uncertainty is included in input
        num_joints = self.input_dim // 3
        uncertainty_dim = num_joints * 3 * 2  # num_joints * coords_per_joint * (var + cov)
        ...
        if input_dim == self.input_dim:
            ...
            input_pose = x[:, :, :self.input_dim]
            # Uncertainty is [batch, seq_len, uncertainty_dim] but Experiment3 uses [batch, uncertainty_dim]
            # Take the last timestep's uncertainty as representative
            input_uncertainty = x[:, -1, self.input_dim:]  # [batch, uncertainty_dim]

        # Pose embedding (NO DCT transform in Experiment3)
        x = nn.Dense(self.d_model, name="input_embed_0")(input_pose)
        ...
        # Parallel uncertainty processing path (Experiment3 style)
        ...
        if input_uncertainty is not None:
            # Process uncertainty: [batch, uncertainty_dim] -> [batch, d_model]
            uncertainty_features = uncertainty_embedding(input_uncertainty)
            # Expand to all timesteps: [batch, d_model] -> [1, batch, d_model] -> [seq_len, batch, d_model]
            uncertainty_features = jnp.expand_dims(uncertainty_features, axis=0)
            uncertainty_features = jnp.tile(uncertainty_features, (self.seq_len, 1, 1))
            # Add to main features (direct addition, no scaling)
        ...
        pred_poses = jnp.concatenate([low_freq_out, high_freq_out], axis=-1)
        # NO IDCT transform in Experiment3!

        # Predict uncertainties (NO gradient detachment in Experiment3!)
        ...
        # Use features WITHOUT detachment (matches Experiment3)
        log_vars, raw_covs = uncertainty_head(features[-1], uncertainty_features)

        # Truncate to output length
        pred_poses = pred_poses[:, :self.seq_len_output, :]
