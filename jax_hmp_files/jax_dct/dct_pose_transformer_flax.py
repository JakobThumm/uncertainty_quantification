"""
Flax/JAX implementation of DCTPoseTransformer
Equivalent to the PyTorch version in models/HMP/transformer_model_and_loss.py
"""
...
class FrequencyAwareAttention(nn.Module):
    ...
    def __call__(self, x):
        ...
        freq_weights = self.param('freq_weights', 
...
class UncertaintyEmbedding(nn.Module):
    ...
    def __call__(self, uncertainty):
        ...
        uncertainty_scale = self.param('uncertainty_scale',
...
class DCTPoseTransformerFlax(nn.Module):
    ...
    def __call__(self, x, input_uncertainty=None):
        ...
        freq_pos_embed = self.param('freq_pos_embed',
                                     nn.initializers.normal(stddev=1.0),
                                     (self.seq_len, 1, self.d_model))
        x = x + freq_pos_embed
        
        # Pass through transformer blocks
        features = []
        for i in range(self.num_layers):
            block = DCTPoseTransformerBlock(
                self.d_model, self.nhead, name=f'transformer_block_{i}'
            )
            x = block(x)
            features.append(x)
        ...
        poses = jnp.concatenate([low_freq_out, high_freq_out], axis=-1)
        
        # Predict uncertainties (using detached features in training)
        num_joints = self.input_dim // 3
        uncertainty_head = UncertaintyHead(
            self.d_model, self.seq_len, self.seq_len_output,
            num_joints=num_joints, coords_per_joint=3,
